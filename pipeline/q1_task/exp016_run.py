"""
EXP-016: extra-long lags +-72/+-96/+-120ч (3-5 дней NWP context) поверх EXP-012.

EXP-007: +-1..12 дал LB 8.0624.
EXP-012: +-18/24/36/48 дал LB 8.0061 (current best).
Гипотеза H_xlong: +-72/+-96/+-120 (3..5 days) даст ещё больше temporal NWP context.

Open-Meteo forecast horizon 7 days = 168 часов -> +-120 безопасно.
NaN на границе valid (~120 строк из 2160) median-filled - допустимо.

Diff от EXP-012: только EXTRA_LAG_HOURS расширен (фильтр rep<=4 тот же).
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-016"
...
# EXP-016: extra-long lags +-72/+-96/+-120 поверх EXP-012 set
EXTRA_LAG_HOURS = [
    -120, -96, -72, -48, -36, -24, -18, -12, -8, -6, -4, -3, -2, -1,
    1, 2, 3, 4, 6, 8, 12, 18, 24, 36, 48, 72, 96, 120,
]
...
def main():
    ...
    print(f"[EXP-016] features: {len(final_cols)} (incl. {len(extra_lag_names)} extra lags)")
    ...
    print(f"\n[EXP-016] CV mean = {cv_mean:.4f}% +/- {cv_std:.4f}%   Q1-only = {cv_q1:.4f}%")
    ...
    summary = {
        "exp_id": "EXP-016", "name": "lgbm_extra_long_lags_72_96_120",
