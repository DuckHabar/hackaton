"""
EXP-066: cross-NWP disagreement features поверх EXP-020 setup.
...
EXP_DIR = ROOT / "experiments/active/EXP-066"
...
must_have = [
    ...
    "n_avail", "month", "hour_of_day", "temperature_80m", "hour_sin", "hour_cos", "month_sin", "month_cos",
...
def main():
    t0 = time.time()
    print("[EXP-066] prepare features ...")
    train_f, valid_f = prepare_features()
    # EXP-066 fresh-eyes #3 fix: cyclic encoding hour/month
    import numpy as np
    for d in [train_f, valid_f]:
        d["hour_sin"] = np.sin(2*np.pi*d["hour_of_day"]/24)
        d["hour_cos"] = np.cos(2*np.pi*d["hour_of_day"]/24)
        d["month_sin"] = np.sin(2*np.pi*d["month"]/12)
        d["month_cos"] = np.cos(2*np.pi*d["month"]/12)
    print("[EXP-066] +4 cyclic features added")
    ...
    print(f"[EXP-066] features: {len(final_cols)} (sister={len(sister_cols)}, dis={len(dis_cols)})")
    ...
    print(f"\n[EXP-066] CV = {cv_mean:.4f}% +/- {cv_std:.4f}% Q1={cv_q1:.4f}%")
    ...
    summary = {
        "exp_id": "EXP-066", "name": "cyclic_encoding_hour_month",
