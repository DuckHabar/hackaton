"""
EXP-033: EXP-021 setup но train ТОЛЬКО на Q1 (jan/feb/mar всех годов = ~7700 rows).

Гипотеза: Q1-specialist лучше generalize на Q1 2026 valid чем общая модель.
Меньше данных но tightly fit к seasonal распределению.

Опасность: GroupKFold по month_key даст только 3*5=15 групп (Q1 2022..2026).
Используем 5 splits вместо 12.
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-033"
...
def main():
    ...
    # Q1-ONLY filter
    is_q1_train = train_f_filtered["dt"].dt.month.isin([1, 2, 3])
    train_f_filtered = train_f_filtered[is_q1_train].copy()
    print(f"[EXP-033] Q1-only training set: {len(train_f_filtered)} rows", flush=True)
    ...
    print(f"[EXP-033] {len(final_cols)} features ...", flush=True)
    ...
    print(f"Unique month_keys: {n_groups}, using {n_splits}-fold GroupKFold", flush=True)
    ...
    print(f"\n[EXP-033] Q1-only specialist CV = {cv_mean:.4f}%", flush=True)
    print(f"vs EXP-021 Q1 (8.351): {8.351-cv_mean:+.4f}", flush=True)
    ...
    summary = {
        "exp_id": "EXP-033", "name": "lgbm_q1_only_specialist",
        ...
        "cv_q1_only_mean_nmae": round(cv_mean, 4),
