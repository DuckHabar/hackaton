"""
EXP-023: оптимальный sister NWP set = 6 sources (gfs+icon+arpege+knmi+dmi+metno).

EXP-020 (5 src без metno): LB 7.3365
EXP-021 (5 src + disagreement): LB 7.3264 prev best
EXP-022 (8 src с bom+cma): LB 7.3704 regressed (bom/cma noise)

Гипотеза: metno useful (CV +0.04 contribution), bom/cma noise.
6 src = 5 + metno -> ожидаем LB 7.30-7.32 (новый best).
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-023"
...
SISTER_SOURCES = ["gfs", "icon", "arpege", "knmi", "dmi", "metno"]
...
def main():
    t0 = time.time()
    print("[EXP-023] prepare features ...")
    ...
    print(f"[EXP-023] features: {len(final_cols)}")
    ...
    print(f"\n[EXP-023] CV = {cv_mean:.4f}% +/- {cv_std:.4f}% Q1={cv_q1:.4f}%")
    print(f"vs EXP-021 (8.242 / 8.351): {8.242-cv_mean:+.4f} / {8.351-cv_q1:+.4f}")
    ...
    summary = {
        "exp_id": "EXP-023", "name": "sister_6src_metno_plus_disagreement",
