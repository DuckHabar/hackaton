"""
EXP-090 PHYS: fork EXP-021 + добавляем explicit P2 interaction features.
"""
...
from common.p2_features import add_p2_interactions
from lgbm_q50 import compute_segments, prepare_features
from nwp_bias_correction import add_p_phys_corrected, apply_bias, estimate_bias
...
P2_FEATS = [
    "is_evening", "is_strict_day", "is_strict_night", "is_dawn",
    "eve_x_rho", "rho_x_ws3_120",
    "night_x_alpha", "day_x_alpha", "night_x_ws120", "day_x_ws120",
    "ws120_excess_10", "ws120_deficit_10", "ws120_excess_14",
    "n_rep_x_high_ws", "is_n_rep_3", "is_n_rep_4", "rep3_x_high_ws",
    "eve_x_ws120", "eve_x_high_ws",
    "temp_grad_80_120", "alpha_10_120",
]
TOP_FEATS = list(dict.fromkeys(top15 + must_have + P2_FEATS))
...
def main():
    t0 = time.time()
    print("="*70)
    print("EXP-090 PHYS: EXP-021 + P2 interaction features")
    print("="*70)

    # Same prep as EXP-021
    print("[EXP-090] prepare features ...")
    ...
    print(f"[EXP-090] base prep: train {train_f.shape}, valid {valid_f.shape}")

    # P2 features
    train_f = add_p2_interactions(train_f)
    valid_f = add_p2_interactions(valid_f)
    print(f"[EXP-090] after P2 features: train {train_f.shape}, valid {valid_f.shape}")

    print(f"[EXP-090] adding extra lags ...")
    combined = pd.concat([train_f.assign(_src="train"), valid_f.assign(_src="valid")], ignore_index=True, sort=False)
    ...
    p2_used = [c for c in P2_FEATS if c in final_cols]
    print(f"[EXP-090] features: {len(final_cols)} (P2={len(p2_used)}, sister={len(sister_cols)}, dis={len(dis_cols)})")
    ...
    print(f"\n[EXP-090] CV = {cv_mean:.4f}% +/- {cv_std:.4f}% Q1={cv_q1:.4f}%")
    print(f"vs EXP-021 (8.238 / 8.241): {8.238-cv_mean:+.4f} / {8.241-cv_q1:+.4f}")
    ...
    pd.DataFrame({"dt": valid_f["dt"].values, "pred_mw": pred_mw_v,
                  "p_phys": valid_f.get("p_phys", valid_f["wind_speed_120m"]).values,
    ...
    print(f"\nTop P2 feature importance:")
    p2_imp = [(f, g) for f, g in imp if f in P2_FEATS][:10]
    for f, g in p2_imp:
        print(f"  {f}: {g:.0f}")
    print(f"\nTop 15 features overall:")
    for f, g in imp[:15]:
        marker = " ** P2" if f in P2_FEATS else ""
        print(f"  {f}: {g:.0f}{marker}")

    summary = {
        "exp_id": "EXP-090", "name": "phys_p2_interactions",
        "n_features": len(final_cols),
        "p2_features_used": p2_used,
    ...
    print(f"\nSummary saved. Time: {summary['elapsed_sec']:.1f}s")
