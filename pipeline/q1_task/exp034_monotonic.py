"""
EXP-034: EXP-021 setup + monotonic constraints LGBM.

Гипотеза: forced monotonicity по физически очевидным features:
  n_repair -> - p  (-1)
  n_avail / available_frac -> + p  (+1)
  wind_speed_120m / ws_corrected_120 / p_phys / p_phys_corrected -> + p  (+1)
  icing_risk -> - p  (-1)

Может стабилизировать predictions + дать interpretability для жюри 25%.
LGBM `monotone_constraints` parameter (list per feature).
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-034"
...
def build_monotone_constraints(cols):
    ...
    INC_PATTERNS = ["wind_speed_120m", "wind_speed_180m", "ws_corrected_120",
                    "ws_corrected_120_corr", "p_phys", "p_phys_corrected",
                    "p_phys_no_density", "rews_simple", "rews_corr", "ws_at_84",
                    "n_avail", "available_frac", "wind_gusts_10m", "wind_speed_80m"]
    DEC_PATTERNS = ["Кол-во_ВЭУ_в_ремонте", "icing_risk"]
    cons = []
    for c in cols:
        ci = 0
        # Avoid disagreement std/range, lag features etc - only base
        if "__std_all" in c or "__range_all" in c or "_xlag_" in c or "__mean_all" in c:
            ci = 0
        elif any(c == p or c.startswith(p + "_") for p in DEC_PATTERNS) or any(p in c for p in DEC_PATTERNS):
            if "Кол-во" in c or "icing" in c:
                ci = -1
        elif any(c == p for p in INC_PATTERNS):
            ci = 1
        cons.append(ci)
    return cons
...
def main():
    ...
    print(f"[EXP-034] {len(final_cols)} features ...", flush=True)

    monotone = build_monotone_constraints(final_cols)
    inc_count = sum(1 for x in monotone if x == 1)
    dec_count = sum(1 for x in monotone if x == -1)
    print(f"Monotonic: {inc_count} increasing, {dec_count} decreasing, {len(monotone)-inc_count-dec_count} free", flush=True)
    ...
    # quantile+monotone не поддерживается LGBM; используем regression_l1 (MAE) ≈ quantile τ=0.5
    hps = {"objective": "regression_l1", "metric": "mae",
    ...
    print(f"\n[EXP-034] monotonic CV = {cv_mean:.4f}% Q1={cv_q1:.4f}%", flush=True)
    ...
    summary = {
        "exp_id": "EXP-034", "name": "lgbm_monotonic_constraints",
        "n_features": len(final_cols), "n_train_clean": len(train_clean),
        "n_inc": inc_count, "n_dec": dec_count,
