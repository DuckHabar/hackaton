"""
EXP-039: EXP-021 setup + Richardson stability + pressure tendency + hub-height TI features.

Добавляем physics-grounded features:
- Ri_proxy = (T_180 - T_80) / (ws_180 - ws_80)^2 atmospheric stability
- dp_3h, dp_6h = pressure_msl tendency (frontal passage)
- TI_proxy = wind_gusts_10m / (ws_10m + 0.5), TI_hub = TI_proxy × (10/120)^0.2
- rolling_ws120_std_6h = std(ws_120, window=6)
- vortex_proxy = ws_180 * sin(wd_180 - wd_80) (directional shear)

Все из существующих ECMWF columns, no API call needed.
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-039"
...
def add_physics_extras(df):
    """Richardson, pressure tendency, TI hub, rolling std, directional shear."""
    df = df.sort_values("dt").reset_index(drop=True)
    NEW = []

    # 1. Atmospheric stability proxy Ri ~ (dT/dz) / (du/dz)^2
    if "temperature_180m" in df.columns and "temperature_80m" in df.columns:
        dT = df["temperature_180m"] - df["temperature_80m"]
        if "wind_speed_180m" in df.columns and "wind_speed_80m" in df.columns:
            dws = (df["wind_speed_180m"] - df["wind_speed_80m"]).replace(0, np.nan)
            df["ri_proxy_80_180"] = dT / (dws ** 2 + 0.01)
            NEW.append("ri_proxy_80_180")
    # 2. Lower-layer stability if temperature_2m exists
    if "temperature_2m" in df.columns and "temperature_80m" in df.columns:
        dT_low = df["temperature_80m"] - df["temperature_2m"]
        if "wind_speed_10m" in df.columns and "wind_speed_80m" in df.columns:
            dws_low = (df["wind_speed_80m"] - df["wind_speed_10m"]).replace(0, np.nan)
            df["ri_proxy_low"] = dT_low / (dws_low ** 2 + 0.01)
            NEW.append("ri_proxy_low")

    # 3. Pressure tendency 3h / 6h
    if "pressure_msl" in df.columns:
        df["dp_3h"] = df["pressure_msl"] - df["pressure_msl"].shift(3)
        df["dp_6h"] = df["pressure_msl"] - df["pressure_msl"].shift(6)
        df["dp_3h_sign"] = np.sign(df["dp_3h"]) * np.sqrt(df["dp_3h"].abs())
        NEW += ["dp_3h", "dp_6h", "dp_3h_sign"]

    # 4. TI proxy and hub-height extrap
    if "wind_gusts_10m" in df.columns and "wind_speed_10m" in df.columns:
        ti_proxy = df["wind_gusts_10m"] / (df["wind_speed_10m"] + 0.5)
        df["ti_proxy_10"] = ti_proxy
        df["ti_hub_proxy"] = ti_proxy * (10.0 / 120.0) ** 0.2
        NEW += ["ti_proxy_10", "ti_hub_proxy"]

    # 5. Rolling std of ws_120 over 6h (turbulence proxy)
    if "wind_speed_120m" in df.columns:
        df["ws_120_rollstd_6h"] = df["wind_speed_120m"].rolling(window=6, min_periods=1).std()
        df["ws_120_rollmean_6h"] = df["wind_speed_120m"].rolling(window=6, min_periods=1).mean()
        df["ws_120_rollrange_6h"] = (df["wind_speed_120m"].rolling(window=6, min_periods=1).max() -
                                     df["wind_speed_120m"].rolling(window=6, min_periods=1).min())
        NEW += ["ws_120_rollstd_6h", "ws_120_rollmean_6h", "ws_120_rollrange_6h"]

    # 6. Directional shear (wd_180 - wd_80) magnitude через sin cos
    if "wind_direction_180m" in df.columns and "wind_direction_80m" in df.columns:
        wd_diff = (df["wind_direction_180m"] - df["wind_direction_80m"]) * np.pi / 180.0
        df["wd_shear_cos"] = np.cos(wd_diff)
        df["wd_shear_sin"] = np.sin(wd_diff)
        NEW += ["wd_shear_cos", "wd_shear_sin"]

    print(f"  add_physics_extras: {len(NEW)} new features: {NEW}", flush=True)
    return df, NEW
...
def main():
    ...
    combined, EXTRA_PHYS = add_physics_extras(combined)
    ...
    all_cols = list(dict.fromkeys(TOP_FEATS + EXTRA_PHYS + extra_lag_names_base + sister_cols + dis_cols))
    final_cols = [c for c in all_cols if c in train_f_filtered.columns]
    print(f"[EXP-039] {len(final_cols)} features (incl {len(EXTRA_PHYS)} new physics) ...", flush=True)
    ...
    print(f"\n[EXP-039] richardson CV = {cv_mean:.4f}% Q1={cv_q1:.4f}%", flush=True)
    ...
    # Top-20 importance включая new physics
    imp = sorted(zip(final_cols, feat_imp), key=lambda kv: -kv[1])[:30]
    print(f"\nTop30 feat importance:")
    for f, g in imp:
        marker = " *NEW*" if f in EXTRA_PHYS else ""
        print(f"  {f}: {g:.0f}{marker}")
    new_phys_imp = [(f, g) for f, g in zip(final_cols, feat_imp) if f in EXTRA_PHYS]
    new_phys_imp = sorted(new_phys_imp, key=lambda kv: -kv[1])
    print(f"\nNew physics features importance:")
    for f, g in new_phys_imp:
        print(f"  {f}: {g:.0f}")

    summary = {
        "exp_id": "EXP-039", "name": "lgbm_richardson_pressure_ti_extras",
        "n_features": len(final_cols), "n_train_clean": len(train_clean),
        "new_physics_features": EXTRA_PHYS,
        "new_features_imp": {f: int(g) for f, g in new_phys_imp},
