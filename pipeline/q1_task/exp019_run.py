"""
EXP-019: расширенный sister NWP stacking (3 sources: GFS + ICON + ARPEGE).

EXP-017 (gfs+icon) = LB 7.6914.
Гипотеза: добавление ARPEGE Europe (Meteo France) как 3-й independent NWP даёт ещё +0.05-0.15 п.п.
ARPEGE имеет 33% NaN на heights (median-fill), но даёт diversity.

Diff от EXP-017: sources=["gfs", "icon", "arpege"] vs ["gfs", "icon"].
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-019"
...
SISTER_SOURCES = ["gfs", "icon", "arpege"]
...
def main():
    t0 = time.time()
    print("[EXP-019] prepare features (ECMWF base) ...")
    ...
    print(f"[EXP-019] after ECMWF lags: train {train_f.shape}")

    print(f"[EXP-019] adding sister NWP features ({SISTER_SOURCES}) ...")
    train_f, valid_f = add_sister_features(train_f, valid_f, sources=SISTER_SOURCES)
    print(f"[EXP-019] after sister: train {train_f.shape}, valid {valid_f.shape}")

    n_repair_col = "Кол-во_ВЭУ_в_ремонте"
    n_repair = train_f[n_repair_col]
    train_f_filtered = train_f[n_repair <= 5].copy()
    print(f"[EXP-019] train filter n_repair<=5: {len(train_f)} -> {len(train_f_filtered)}")
    ...
    print(f"[EXP-019] features total: {len(final_cols)} (sister={len(sister_cols)})")
    ...
    bp.setdefault("feature_fraction", 0.85)
    bp["feature_fraction"] = min(bp.get("feature_fraction", 0.85), 0.8)  # 80% при ~580 cols
    ...
    print(f"\n[EXP-019] CV mean = {cv_mean:.4f}% +/- {cv_std:.4f}%   Q1-only = {cv_q1:.4f}%")
    print(f"vs EXP-017 (8.720 / 8.931): {8.720-cv_mean:+.4f} / {8.931-cv_q1:+.4f}")
    ...
    sister_imp = [(f, g) for f, g in imp if any(f"__{s}" in f for s in SISTER_SOURCES)]
    print(f"\nTop sister features by gain (per source):")
    for src in SISTER_SOURCES:
        s_imp = [(f, g) for f, g in imp if f"__{src}" in f][:3]
        print(f"  {src}:")
        for f, g in s_imp:
            print(f"    {f}: {g:.0f}")

    summary = {
        "exp_id": "EXP-019", "name": "sister_nwp_stacking_gfs_icon_arpege_3src",
