"""
EXP-067: cross-NWP disagreement features поверх EXP-020 setup.
...
EXP_DIR = ROOT / "experiments/active/EXP-067"
...
def main():
    t0 = time.time()
    print("[EXP-067] prepare features ...")
    ...
    print(f"[EXP-067] features: {len(final_cols)} (sister={len(sister_cols)}, dis={len(dis_cols)})")
    ...
    for fold, (tr, te) in enumerate(gkf.split(X, y, groups)):
        ...
        pred_mw = np.clip(pred, 0.0, 90.09)  # EXP-067 fresh-eyes #1 fix: cap to P_INST not n_avail*rated
        oof[te] = pred_mw
    ...
    print(f"\n[EXP-067] CV = {cv_mean:.4f}% +/- {cv_std:.4f}% Q1={cv_q1:.4f}%")
    ...
    pred_mw_v = np.clip(preds, 0.0, 90.09)
    ...
    summary = {
        "exp_id": "EXP-067", "name": "n_avail_cap_to_p_inst",
