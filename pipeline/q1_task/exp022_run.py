"""
EXP-022: sister NWP stacking с 8 sources (max diversity).

EXP-020 (5 src) -> LB 7.3365.
EXP-022 расширяет: + metno (Norway), + bom (Australia), + cma (China).
- metno 100% coverage 120m (top kandidat)
- bom 34% (added but heavily NaN-filled)
- cma 50% (partial diversity)

Гипотеза: ещё +0.05-0.15 п.п. -> LB 7.2-7.3, потенциально BREAK global best 7.282.
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-022"
...
SISTER_SOURCES = ["gfs", "icon", "arpege", "knmi", "dmi", "metno", "bom", "cma"]
...
def main():
    t0 = time.time()
    print("[EXP-022] prepare features ...")
    ...
    print(f"[EXP-022] adding 8 sister sources ...")
    train_f, valid_f = add_sister_features(train_f, valid_f, sources=SISTER_SOURCES)
    print(f"[EXP-022] train {train_f.shape}, valid {valid_f.shape}")
    ...
    print(f"[EXP-022] features: {len(final_cols)} (sister={len(sister_cols)})")
    ...
    bp["feature_fraction"] = 0.65  # ~1400 cols, нужна сильная регуляризация
    bp["min_data_in_leaf"] = 300
    ...
    print(f"\n[EXP-022] CV = {cv_mean:.4f}% +/- {cv_std:.4f}% Q1={cv_q1:.4f}%")
    ...
    print(f"\nTop sister features per source:")
    for src in SISTER_SOURCES:
        s_imp = [(f, g) for f, g in imp if f"__{src}" in f][:2]
        print(f"  {src}: {[(f, int(g)) for f, g in s_imp]}")

    summary = {
        "exp_id": "EXP-022", "name": "sister_8src_max_diversity",
        "n_features": len(final_cols), "n_train_clean": len(train_clean),
        "sister_sources": SISTER_SOURCES, "n_sister_features": len(sister_cols),
