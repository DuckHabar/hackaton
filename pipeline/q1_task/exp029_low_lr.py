"""
EXP-029: EXP-021 setup но с low learning rate 0.01 + 5000 iters + strong regularization.

Гипотеза: slower learning + сильная regularization -> меньше overfit на train, лучше generalize на LB.
EXP-021 LB 7.3264 был с lr=0.034 (default optuna). EXP-029 lr=0.01 + min_data_in_leaf=400.
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-029"
...
def main():
    ...
    print(f"[EXP-029] {len(final_cols)} features ...", flush=True)
    ...
    # EXP-029 HPs: low lr, strong reg, more iters
    hps = {
    ...
    print(f"\n[EXP-029] LGBM low-lr CV = {cv_mean:.4f}% Q1={cv_q1:.4f}%", flush=True)
    ...
    summary = {
        "exp_id": "EXP-029", "name": "lgbm_low_lr_strong_reg",
        "n_features": len(final_cols), "n_train_clean": len(train_clean),
        "hps": hps, "n_boost": n_boost,
