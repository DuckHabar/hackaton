"""
EXP-080: train без Q1 2025 (contrarian #1 direct LB probe).

Hypothesis: 2026 ≈ pre-2025 distribution. Если так - drop Q1 2025 train rows
улучшит LB но ухудшит CV (т.к. CV считается на Q1 2025).

EXP-021 setup, но train_f filtered: НЕТ Q1 2025.
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-080"
...
def main():
    t0 = time.time()
    print("[EXP-080] prepare features ...")
    ...
    # === DROP Q1 2025 ===
    drop_mask = (train_f_filt["dt"].dt.year == 2025) & train_f_filt["dt"].dt.month.isin([1, 2, 3])
    print(f"Q1 2025 rows to drop: {drop_mask.sum()} ({drop_mask.sum()/len(train_f_filt)*100:.1f}% of train)")
    train_f_filt = train_f_filt[~drop_mask].copy()
    ...
    print(f"\n[EXP-080] CV (no Q1 2025) = {cv_mean:.4f}% +/- {cv_std:.4f}%")
    ...
    summary = {
        "exp_id": "EXP-080", "name": "drop_q1_2025_train",
        "n_features": len(final_cols), "n_train_after_drop": len(train_clean),
