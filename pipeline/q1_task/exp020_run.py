"""
EXP-020: max sister NWP stacking - 5 sources: GFS + ICON + ARPEGE + KNMI + DMI.

EXP-017 (2 src): LB 7.6914
EXP-019 (3 src): CV ~8.63 (LB pending)
Гипотеза: 5 NWP sources -> ещё +0.05-0.10 п.п. LB. Точка saturation возможна, но user
правило 'сабмитеть всё' - пробуем.

KNMI (Netherlands MO) и DMI (Denmark MO) - оба seamless модели, 100% coverage 120m.
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-020"
...
def main():
    t0 = time.time()
    print("[EXP-020] prepare features ...")
    ...
    print(f"[EXP-020] adding 5 sister sources {SISTER_SOURCES} ...")
    train_f, valid_f = add_sister_features(train_f, valid_f, sources=SISTER_SOURCES)
    print(f"[EXP-020] after sister: train {train_f.shape}, valid {valid_f.shape}")
    ...
    print(f"[EXP-020] features total: {len(final_cols)} (sister={len(sister_cols)})")
    ...
    bp["feature_fraction"] = 0.75  # ~900 cols, нужно больше регуляризации
    ...
    print(f"\n[EXP-020] CV = {cv_mean:.4f}% +/- {cv_std:.4f}% Q1={cv_q1:.4f}%")
    print(f"vs EXP-019 (8.629 / 8.813): {8.629-cv_mean:+.4f} / {8.813-cv_q1:+.4f}")
    ...
    summary = {
        "exp_id": "EXP-020", "name": "sister_5src_gfs_icon_arpege_knmi_dmi",
