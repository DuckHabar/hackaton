"""
EXP-025: multi-seed ensemble - train EXP-021 setup с 5 seeds, average predictions.

Variance reduction: каждый seed даёт чуть другие feature splits и OOF.
Усреднение должно дать +0.02-0.05 п.п. над EXP-021.

Seeds: [42, 13, 7, 99, 31]. Total runtime ~6-8 min.
"""
...
EXP_DIR = ROOT / "experiments/active/EXP-025"
...
SISTER_SOURCES = ["gfs", "icon", "arpege", "knmi", "dmi"]  # EXP-021 set
SEEDS = [42, 13, 7, 99, 31]
...
def main():
    t0 = time.time()
    print("[EXP-025] prepare features (load once) ...")
    ...
    print(f"[EXP-025] features ready: train {train_f.shape}, valid {valid_f.shape}")
    ...
    print(f"[EXP-025] {len(final_cols)} features, training {len(SEEDS)} seeds ...")
    ...
    for seed in SEEDS:
        ...
        hps = {"objective": "quantile", "alpha": 0.5, "metric": "quantile",
               "verbose": -1, "num_threads": 0, "device": "cpu", "seed": seed,
        ...
        for fold, (tr, te) in enumerate(gkf.split(X, y, groups)):
            ...
            vp = m.predict(Xv, num_iteration=m.best_iteration)
            valid_preds.append(vp)
        seed_valid = np.mean(valid_preds, axis=0)
        seed_valid_clip = np.clip(seed_valid, 0.0, nv_avail * TURBINE_RATED_MW)
        all_oof.append(oof)
        all_valid_pred.append(seed_valid_clip)
        cv_seed = float(np.mean(fold_nmae))
        is_q1_v = train_clean["dt"].dt.month.isin([1,2,3]).values
        cv_q1_seed = float(nmae(y[is_q1_v], oof[is_q1_v]))
        print(f"  seed {seed}: CV={cv_seed:.4f}% Q1={cv_q1_seed:.4f}% ({time.time()-ts:.1f}s)")
        seed_summaries.append({"seed": seed, "cv": round(cv_seed, 4), "cv_q1": round(cv_q1_seed, 4)})

    oof_ens = np.mean(all_oof, axis=0)
    valid_ens = np.mean(all_valid_pred, axis=0)

    is_q1_v = train_clean["dt"].dt.month.isin([1,2,3]).values
    cv_ens = float(nmae(y, oof_ens))
    cv_q1_ens = float(nmae(y[is_q1_v], oof_ens[is_q1_v]))
    print(f"\n[EXP-025] ENSEMBLE OOF: CV={cv_ens:.4f}% Q1={cv_q1_ens:.4f}%")
    print(f"vs EXP-021 (8.242 / 8.351): {8.242-cv_ens:+.4f} / {8.351-cv_q1_ens:+.4f}")

    pd.DataFrame({"dt": valid_f["dt"].values, "pred_mw": valid_ens,
                  "p_phys": valid_f["p_phys"].values, "n_avail": nv_avail}).to_parquet(EXP_DIR / "valid_pred.parquet")
    pd.DataFrame({"dt": train_clean["dt"].values, "y_true": y, "oof_pred": oof_ens}).to_parquet(EXP_DIR / "oof.parquet")

    summary = {
        "exp_id": "EXP-025", "name": "exp021_multiseed_5x",
        "base": "EXP-021", "seeds": SEEDS,
        "n_features": len(final_cols),
        "seed_summaries": seed_summaries,
        "cv_mean_nmae": round(cv_ens, 4),
        "cv_q1_only_mean_nmae": round(cv_q1_ens, 4),
