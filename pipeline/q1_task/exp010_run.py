"""
EXP-010: relaxed filter - drop только rep>=6 (только May 2022/2023/2024), оставить rep_5.

Гипотеза: rep_5 - это всё ещё «нормальный» режим (n_avail=21 близко к valid n_avail=22-23),
он мог помогать модели через дополнительные observations. EXP-009 (drop rep≥5) был
слишком агрессивным - теряем 9932 строк включая полезные Aug/Nov/Dec months.

EXP-010 dropит только May 2022/2023/2024 (4 месяца, ~2900 ч с n_repair≥6).
Если LB ≤ 8.0494 -> EXP-010 оптимальное.
Если LB между 8.0494 и 8.0624 -> rep_5 нейтрален.
"""
import json
import sys
import time
...
EXP_DIR = ROOT / "experiments/active/EXP-010"
...
def main():
    ...
    # RELAXED FILTER: drop только rep≥6 (только May 2022/2023/2024)
    train_f_filtered = train_f[n_repair <= 5].copy()
    n_after = len(train_f_filtered)
    dropped = train_f[n_repair >= 6]
    dropped_months = sorted(dropped["dt"].dt.strftime("%Y-%m").unique().tolist())
    print(f"[EXP-010] Train reshape: {n_before} -> {n_after} (dropped {n_before-n_after}); months: {dropped_months}")
    ...
    print(f"\n[EXP-010] CV mean = {cv_mean:.4f}% ± {cv_std:.4f}%   Q1-only = {cv_q1:.4f}%")
    ...
    summary = {
        "exp_id": "EXP-010",
        "name": "lgbm_relaxed_filter_drop_rep6plus",
        "n_features": len(final_cols),
        "n_train_before": n_before, "n_train_after": n_after,
        "n_train_clean": len(train_clean),
        "dropped_months": dropped_months,
        "sw_q1_factor": sw_q1, "sw_rep4_factor": 1.2,
